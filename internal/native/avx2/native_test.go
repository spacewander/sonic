...
func Test_parse_lazy(t *testing.T) {
	tests := []struct {
		name    string
		s    string
		path []interface{}
	    ...
	    {"self", `true`, []interface{}{}, 0, 4, types.NewNode(`true`, 0, 0)},
	    {"array", `[1,2]`, []interface{}{1}, 3, 4, types.NewNode(`2`, 0, 0)},
	    {"object", `{"a":1}`, []interface{}{"a"}, 5, 6, types.NewNode(`1`, 0, 0)},
	    {"object-array", `{"a":[1]}`, []interface{}{"a",0}, 6, 7, types.NewNode(`1`, 0, 0)},
	    {"array-object-array", `[{"a":[1]}]`, []interface{}{0,"a",0}, 7, 8, types.NewNode(`1`, 0, 0)},
	    {"array-object", `[{"a":["\""]}]`, []interface{}{0,"a"}, 6, 12, types.Node{
            Kind: types.T_ARRAY,
            Flag: 0,
            JSON: `["\""]`,
            Kids: []types.Token{{types.T_STRING, types.F_ESC, 1, 2}},
        }},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
            var n types.Node
            var p int
            gotRet := parse_lazy(&tt.s, &p, &n, &tt.path) 
            println(tt.name, gotRet)
            for gotRet == -types.MUST_RETRY {
                n.Grow()
                gotRet = parse_lazy(&tt.s, &p, &n, &tt.path) 
            }
            
			if gotRet != tt.ret {
				t.Errorf("parse_lazy() = %v, want %v", gotRet, tt.ret)
			}
            if tt.pp != p {
                t.Errorf("exp: %#v, got: %#v", tt.pp, p)
            }
            if !reflect.DeepEqual(tt.node, n) {
                t.Errorf("exp: %#v, got: %#v", tt.node, n)
            }
		})
	}
}
